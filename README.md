...
ドロップすると、**話者ごとに分けるかどうかを1回だけ聞かれます**。
...
ソロ配信なら Enter を押すだけです。コラボなら**人数を入れてください**
（`a` より確実です。理由は下の「コラボ配信」の項）。
...
ドラッグ＆ドロップで使う場合は、聞かれたときに人数を入れてください。
...
合成音声で作った会話（1人／2人／3人／4人×2本）で測った結果です。
...
- **2人までは実用十分。3人以上は1〜3割が別の人のファイルに混ざります**
- `auto` は3人以上で人数を外しやすいので、**分かっているなら数字で指定**してください
- 誤っても字幕の文言は正しく、入るファイルが違うだけです
- 同時に喋っている部分や、相づちのような短い発言は取りこぼします
- 話者判別のぶん処理時間は伸びます（2分15秒の動画で 24秒 → 36秒 程度）

> 上の数字は macOS の合成音声によるものです。声質が似たものが混ざっているため、
> 実際の人間の声では変わります。コラボ動画を1本流して確かめてください。

---

## 音声が複数トラックある動画

マイクを人ごとに分けて録っている場合（OBS のマルチトラック録音、Discord の録音bot など）、
**すべてのトラックを読んで、トラックごとに字幕を作ります**。

さらに、1つのトラックに複数人が入っていても、その中で話者を分けます。

```bash
uv run koewake コラボ配信.mkv --speakers auto
```

```
コラボ配信_Aさんマイク.srt              ← トラック1（1人）
コラボ配信_BCさんマイク_話者1.srt        ← トラック2に2人いた
コラボ配信_BCさんマイク_話者2.srt
```

トラック名は動画に埋め込まれていればそれを使い、無ければ `トラック1` `トラック2` になります。

**トラックごとに人数を指定する**こともできます（カンマ区切り、トラックの順）。

```bash
uv run koewake コラボ配信.mkv --speakers 1,2    # トラック1は1人、トラック2は2人
```

名前を付けるときは、**トラックの順 → 話者の順**でカンマ区切りに並べます。

```bash
uv run koewake コラボ配信.mkv --speakers auto --speaker-names Aさん,Bさん,Cさん
# -> コラボ配信_Aさん.srt / コラボ配信_Bさん.srt / コラボ配信_Cさん.srt
```

> `--speakers` を付けなければ、話者では分けずに**トラックごとに1本ずつ**作ります。
> トラックが1本だけの動画は、これまでどおりの動きです。
...
| `--speakers` | off | `auto` か人数。話者ごとに別々のSRTを出す。`1,2` でトラックごとの指定 |
| `--speaker-names` | なし | 話者につける名前（トラック順→喋り始めた順、カンマ区切り） |
